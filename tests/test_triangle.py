...
def test_triangle_type_values():
    assert int(VertexType.TRIANGLE_INPUT) == 7
    assert int(VertexType.TRIANGLE_OUTPUT) == 8


def test_vertex_is_triangle():
    assert vertex_is_triangle(VertexType.TRIANGLE_INPUT)
    assert vertex_is_triangle(VertexType.TRIANGLE_OUTPUT)
    assert not vertex_is_triangle(VertexType.Z)
    assert not vertex_is_triangle(VertexType.W_OUTPUT)


def test_triangle_partner_and_io():
    g, tip, body = _triangle_graph()
...
def _triangle_map(input_first: bool):
    """1-in-1-out graph containing a single triangle.
    ...
    if input_first:
        ...
        body = g.add_vertex(VertexType.TRIANGLE_OUTPUT, 0, 1)
        tip = g.add_vertex(VertexType.TRIANGLE_INPUT, 0, 2)
...
def test_triangle_matrix():
    m = _triangle_map(input_first=True).to_matrix()
    assert np.allclose(m, np.array([[1, 1], [0, 1]]))


def test_triangle_matrix_layout_independent():
    m = _triangle_map(input_first=False).to_matrix()
    assert np.allclose(m, np.array([[1, 1], [0, 1]]))


def test_triangle_composition():
    g = Graph('multigraph')
    g.set_auto_simplify(False)
    inp = g.add_vertex(VertexType.BOUNDARY, 0, 0)
    tip1 = g.add_vertex(VertexType.TRIANGLE_INPUT, 0, 1)
    body1 = g.add_vertex(VertexType.TRIANGLE_OUTPUT, 0, 2)
    tip2 = g.add_vertex(VertexType.TRIANGLE_INPUT, 0, 3)
    body2 = g.add_vertex(VertexType.TRIANGLE_OUTPUT, 0, 4)
...
def test_triangle_json_roundtrip():
    g, tip, body = _triangle_graph()
    ...
    # the internal W_IO edge survives
    assert any(g2.edge_type(e) == EdgeType.W_IO for e in g2.edges())


def test_triangle_tikz_roundtrip():
    g, tip, body = _triangle_graph()
    g.set_inputs(())
    g.set_outputs(())
    s = zxtikz.to_tikz(g)
